...
that is the whole point of the seam. Proving the evaluation pipeline handles 1000 services' worth of
series does not require 1000 processes, any more than testing a JSON parser requires a real web
server.
...
**Phrase it as "1,000 synthetic service series", never "1,000 services".** Someone will ask how you
ran a thousand JVMs on a laptop. Volunteering the exporter trick is a good moment; being caught by
it is not.
...
### `SYNTHETIC_SERVICES` — the ramp: 100 / 500 / 1000
...
| ~~2,000~~ | ~~4,000~~ | ~~166k~~ | ~~60k~~ | ~~~3–4 GB~~ |

RSS is an estimate, not measured. **2000 is dropped deliberately**, for three reasons in increasing
order of importance:

1. **It does not fit comfortably.** On a 16GB laptop with a 10GB Docker allocation, Postgres,
   Redpanda, Redis, Sentinel and Grafana already take ~3GB.
2. **It would not find the ceiling either.** See §6 — the query count is constant in N, so if p99
   at 1000 is two orders of magnitude below the 15s interval, doubling N leaves it one order below.
   The fifth point confirms the flat line rather than terminating it.
...
Trading the 2000-service step for doubled duration at three sizes costs nothing in coverage and buys
a materially more defensible tail.
...
High enough to be a genuine storm — at 1000 services that is 300 services breaching inside one
...
- `SloEvaluator` logging at `ERROR` — at 1000 services with 30% breaching, WARN-level breach logging
  is 600 synchronous lines per cycle *on the thread being timed*. Measurement apparatus inside the
  measurement.
...
SIZES="100 500 1000" DURATION_MIN=20 ./scripts/load-test.sh
...
K6="docker run --rm --network sentinel_default -v $(pwd)/loadtest/k6:/scripts:ro \
...
$K6 run -e FRACTION=0.3 /scripts/breach-storm.js

# 5 — duplicate replay                                             ~5 min
$K6 run -e COUNT=10000 /scripts/duplicate-replay.js
...
step; if Prometheus stays under 2GB at 1000 services, you had headroom for 2000 after all.
...
**Where 1,000 sits.** Most organisations run 50–200 services. A thousand is Uber/Netflix territory.
Most portfolio projects in this space test with three containers and publish no numbers at all. So
2,000 SLOs on a 15s cycle is comfortably above the bar for a personal project.
...
> Evaluated 2,000 SLOs across 1,000 synthetic service series at a 15s interval, p99 cycle latency
> ≤ X ms; correlated cascading failures at an A:B alert-collapse ratio; zero duplicate incidents
> across 10,000 replayed events.

**What not to write:** "ran a 1,000-service fleet", "scales to 1,000+ services" (unmeasured beyond
1,000), or any figure without the hardware it came from.
...
four real fleet services exist for the cascade demo, because that is the part synthetic series
cannot tell you.
...
This is the part worth being rigorous about, because "it was fast at 1000 so it will be fine at
...
2. **Garbage collection.** Response payloads grow linearly, so at some N the per-cycle allocation
   changes GC behaviour and the curve bends upward. Not visible at 1,000; it is the first thing to
...
> It is designed for the scale I measured it at — 1,000 synthetic service series and 2,000 SLOs on a
...
## 8. Where the numbers go afterwards

1. Transcribe from `docs/LOAD_TEST_RESULTS.raw.md` into
   [`LOAD_TEST_RESULTS.md`](LOAD_TEST_RESULTS.md), filling the Environment block.
2. **Delete the `STATUS: NOT YET MEASURED` banner.** It is load-bearing right now; leaving it above
   real numbers is worse than never having run the tests.
3. If the curve is flat — which the model in §6 predicts — do not leave the ceiling line blank or
   guess. Write:

   > Flat from 100 to 1,000 services (p99 ≤ X ms against a 15s interval). **Ceiling not reached** —
   > cycle cost is dominated by response parsing rather than query count, because burn rate is
   > precomputed in recording rules. The test rig ran out of memory before the evaluator ran out of
   > headroom.

4. Put the measured ceiling — or the honest "not reached" — at the front of
   [`SCALING.md`](SCALING.md).
5. Only then write the résumé bullets. The build spec is explicit about the ordering, and it is the
   right ordering: the numbers constrain the sentence, not the other way round.
